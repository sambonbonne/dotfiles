...
#PROMPT_NBSP=$'\u00A0'
PROMPT_NBSP=' '
PROMPT_STATE_SEPARATOR=" ${COLOR_WHITE}❭${COLOR_RESET}${PROMPT_NBSP}"
...
build_prompt() {
  ...
  if [ "${DISTROBOX_ENTER_PATH}" != "" ] || [ "${TOOLBOX_PATH}" != "" ]; then
    echo -n "${PROMPT_NBSP}${COLOR_CYAN}${PROMPT_HOSTNAME}${COLOR_RESET}${PROMPT_STATE_SEPARATOR}"
  fi
  ...
  # if env | grep container >/dev/null 2>&1; then
  #   echo -n "▢${COLOR_CYAN}${PROMPT_HOSTNAME}${COLOR_RESET}${PROMPT_STATE_SEPARATOR}"
  # fi
  ...
  test "${git_state}" != "" && echo -n "${PROMPT_STATE_SEPARATOR}${PROMPT_NBSP}${git_state}"

  test "${VIRTUAL_ENV}" != "" && echo -n "${PROMPT_STATE_SEPARATOR}${PROMPT_NBSP}${COLOR_YELLOW}$(basename "${VIRTUAL_ENV}")${COLOR_RESET}"
